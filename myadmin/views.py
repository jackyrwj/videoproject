...
from video.models import Video, Classification, VideoUser
...
class CouponListView(AdminUserRequiredMixin, generic.ListView):
    model = VideoUser
    template_name = 'myadmin/coupon_list.html'
    ...
    def get_queryset(self):
        self.q = self.request.GET.get("q", "")
        return VideoUser.objects.filter(id__contains=self.q).order_by('-id')
